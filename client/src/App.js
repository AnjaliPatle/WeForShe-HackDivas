  ...
  let landingHTML=<>
    ...
    <div className="o-wrapper-l">
        <div className="hero flex flex-column">
            ...
            <div>
                <div className="actionText">Share this code with your friends</div>
                <span className={copied?"username highlight copied":"username highlight"} onClick={()=>{showCopiedMessage()}}>{yourID}</span>
            </div>
            <div className="callBox">
                <input type="text" placeholder="Enter Call Code" value={receiverID} onChange={e => setReceiverID(e.target.value)} className="form-input"/>
            </div>
            <button onClick={() => callPeer(receiverID.toLowerCase().trim())} className="primaryButton">Call</button>
            <div>
                To start group shopping, share this code with your friend  and wait for their call <span style={{fontWeight: 600}}>OR</span> enter their code and hit call!
            </div>
        </div>
    </div>
    </div>
